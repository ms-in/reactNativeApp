import React, {Component} from 'react';
import {Animated, StyleSheet, Text, TouchableOpacity} from 'react-native';
import {BRAND_BLUE, GRAY_MEDIUM, WHITE} from '../../../styles/colors';
import PropTypes from 'prop-types';

class Toggle extends Component {
  static defaultProps = {
    switchOn: false,
    onPress: () => {},
    containerStyle: {
      width: 60,
      height: 20,
      borderRadius: 10,
      padding: 0,
    },
    circleStyle: {
      width: 24,
      height: 24,
      borderRadius: 12,
      backgroundColor: WHITE, // rgb(102,134,205)
      borderWidth: 1,
      borderColor: GRAY_MEDIUM,
    },
    backgroundColorOn: BRAND_BLUE,
    backgroundColorOff: GRAY_MEDIUM,
    circleColorOff: WHITE,
    circleColorOn: WHITE,
    duration: 300,
    // leftContainerStyle:{
    //     padding: 0,
    //     width: 0
    // },
    textLeftStyle: {
      color: WHITE,
    },
  };

  static propTypes = {
    switchOn: PropTypes.bool,
    onPress: PropTypes.func,
    containerStyle: PropTypes.any,
    circleStyle: PropTypes.any,
    backgroundColorOff: PropTypes.string,
    backgroundColorOn: PropTypes.string,
    circleColorOff: PropTypes.string,
    circleColorOn: PropTypes.string,
    duration: PropTypes.number,
    type: PropTypes.number,

    buttonText: PropTypes.string,
    backTextRight: PropTypes.string,
    backTextLeft: PropTypes.string,

    buttonTextStyle: PropTypes.any,
    textRightStyle: PropTypes.any,
    textLeftStyle: PropTypes.any,

    buttonContainerStyle: PropTypes.any,
    rightContainerStyle: PropTypes.any,
    leftContainerStyle: PropTypes.any,
  };

  componentDidUpdate(prevProps) {
    if (prevProps.switchOn !== this.props.switchOn) {
      this.runAnimation();
    }
  }

  onPress = () => {
    this.props.onPress();
  };

  getStart = () => {
    return this.props.type === undefined
      ? 0
      : this.props.type === 0
      ? 0
      : this.props.containerStyle.padding * 2;
  };

  runAnimation = () => {
    // this.state.anim.setValue(0);
    const animValue = {
      fromValue: this.props.switchOn ? 0 : 1,
      toValue: this.props.switchOn ? 1 : 0,
      duration: this.props.duration,
    };

    Animated.timing(this.state.animXValue, animValue).start();
    // Animated.timing(this.state.anim, animValue).start(() => this.runAnimation());
  };

  constructor(props) {
    super(props);
    const endPos =
      this.props.containerStyle.width -
      (this.props.circleStyle.width + this.props.containerStyle.padding * 2);
    this.state = {
      circlePosXStart: this.getStart(),
      circlePosXEnd: endPos,
      animXValue: new Animated.Value(this.props.switchOn ? 1 : 0),
    };
  }

  render() {
    return (
      <TouchableOpacity testID="toggle" onPress={this.onPress} activeOpacity={0.5}>
        <Animated.View
          style={[
            styles.container,
            this.props.containerStyle,
            {
              backgroundColor: this.state.animXValue.interpolate({
                inputRange: [0, 1],
                outputRange: [
                  this.props.backgroundColorOff,
                  this.props.backgroundColorOn,
                ],
              }),
            },
          ]}
          testID={this.props.testID}>
          {this.props.switchOn && this.generateLeftText()}
          <Animated.View
            style={[
              this.props.circleStyle,
              {
                backgroundColor: this.state.animXValue.interpolate({
                  inputRange: [0, 1],
                  outputRange: [this.props.circleColorOff, this.props.circleColorOn],
                }),
              },
              {
                transform: [
                  {
                    translateX: this.state.animXValue.interpolate({
                      inputRange: [0, 1],
                      outputRange: [this.state.circlePosXStart, this.state.circlePosXEnd],
                    }),
                  },
                ],
              },
            ]}>
            <Animated.View style={this.props.buttonContainerStyle}>
              <Text style={this.props.buttonTextStyle}>{this.props.buttonText}</Text>
            </Animated.View>
          </Animated.View>
          {this.props.switchOff && this.generateRightText()}
        </Animated.View>
      </TouchableOpacity>
    );
  }

  generateRightText() {
    return (
      <Animated.View style={[this.props.rightContainerStyle, styles.rightTextStyle]}>
        <Text style={this.props.textRightStyle}>{this.props.backTextRight}</Text>
      </Animated.View>
    );
  }

  generateLeftText() {
    return (
      <Animated.View style={[this.props.leftContainerStyle, styles.leftTextStyle]}>
        <Text style={this.props.textLeftStyle}>{this.props.backTextLeft}</Text>
      </Animated.View>
    );
  }
}

const styles = StyleSheet.create({
  container: {
    flexDirection: 'row',
    alignItems: 'center',
  },
  leftTextStyle: {
    flexDirection: 'row',
    position: 'absolute',
    left: '20%',
    top: -1,
  },
  rightTextStyle: {
    flexDirection: 'row',
    position: 'absolute',
    right: '20%',
  },
});

export default Toggle;
